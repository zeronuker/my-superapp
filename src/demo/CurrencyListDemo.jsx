...
import { CURRENCIES, CURRENCY_BY_CODE } from '../data/currencyCountries.js'
import { useCalculatorStore } from '../store/calculatorStore'
import ResetButton from '../components/ResetButton.jsx'

// ── PROTOTYPE — isolated sample for review. Does not touch the real
//    CurrencyCalculator.jsx, App.jsx, or calculatorStore.js. Its own
//    localStorage keys (cb-demo-*) are separate from the live app's. ──

const PREFS_KEY = 'cb-demo-currency-prefs'
const RATES_KEY_PREFIX = 'cb-demo-rates-'
...
function loadPrefs() {
  try {
    ...
    if (!raw) return { base: DEFAULT_BASE, list: DEFAULT_LIST }
    const parsed = JSON.parse(raw)
    return {
      base: parsed.base || DEFAULT_BASE,
      list: Array.isArray(parsed.list) && parsed.list.length ? parsed.list : DEFAULT_LIST,
    }
  } catch { return { base: DEFAULT_BASE, list: DEFAULT_LIST } }
}
function savePrefs(base, list) {
  try { localStorage.setItem(PREFS_KEY, JSON.stringify({ base, list })) } catch {}
}

function loadRatesCache(base) {
  try { const r = localStorage.getItem(RATES_KEY_PREFIX + base); return r ? JSON.parse(r) : null }
  catch { return null }
}
function saveRatesCache(base, rates, date) {
  try { localStorage.setItem(RATES_KEY_PREFIX + base, JSON.stringify({ rates, date, fetchedAt: Date.now() })) } catch {}
}
...
export default function CurrencyListDemo() {
  const { settings } = useCalculatorStore()
  const prefs = useMemo(loadPrefs, [])

  const [amount, setAmount] = useState('100')
  const [base, setBase] = useState(prefs.base)
  const [list, setList] = useState(prefs.list)
  ...
  useEffect(() => { savePrefs(base, list) }, [base, list])
  ...
  const handleReset = () => {
    setAmount('100'); setBase(DEFAULT_BASE); setList(DEFAULT_LIST)
  }
  ...
  const removeFromList = (code) => setList(l => l.filter(c => c !== code))
  const toggleInList = (code) => setList(l => l.includes(code) ? l.filter(c => c !== code) : [...l, code])

  const selectBase = (code) => {
    setBase(code)
    setList(l => l.filter(c => c !== code)) // base can't also appear in the output list
